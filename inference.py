...
from src.transformer.transformer import Transformer, create_padding_mask
...
class TranslationInference:
    ...
    def _greedy_decode(self, src_text, max_length):
        src_tokens = [self.src_tokenizer.bos_id()] + self.src_tokenizer.encode(src_text, out_type=int) + [self.src_tokenizer.eos_id()]
        ...
        for _ in range(max_length):
            ...
            for beam_tokens, beam_score in beams:
                ...
                for i in range(beam_size):
                    ...
                    new_beam_tokens = torch.cat([beam_tokens, torch.tensor([[next_token]], dtype=torch.long).to(self.device)], dim=1)
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="./model/best_model.pt", help="Path to the trained model")
    parser.add_argument("--text", type=str, required=True, help="Chinese text to translate")
    parser.add_argument("--beam_size", type=int, default=1, help="Beam size for beam search (1 for greedy)")
